...
파일 체크:
- `_workspace/00_profile_report.md` → 프로파일링 완료 여부
- `_workspace/01_db_done.md` → DB 최적화 완료 여부
- `_workspace/02_frontend_done.md` → 프론트 최적화 완료 여부
- `_workspace/03_qa_report.md` → QA 완료 여부
...
Agent(
  ...
  prompt: "perf-profiler 에이전트로서 작업한다.
           ...
           _workspace/ 디렉토리를 만들고 00_profile_report.md에 진단 결과를 기록하라."
)
```

완료 조건: `_workspace/00_profile_report.md` 존재
...
입력: _workspace/00_profile_report.md 읽기
작업: 
...
출력: _workspace/02_frontend_done.md
```

두 에이전트는 수정 파일이 겹치지 않으므로 완전 병렬 실행 가능.

완료 조건: `_workspace/01_db_done.md` + `_workspace/02_frontend_done.md` 존재
...
## 에러 핸들링
- db-optimizer 실패: `_workspace/01_db_error.md` 기록 후 QA에서 보고
- frontend-optimizer 실패: `_workspace/02_frontend_error.md` 기록 후 QA에서 보고
